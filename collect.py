"""
더즌(462860) 기업분석 리포트 생성기 v8.3
===================================================
- 오류 수정: TypeError(int64) 해결을 위한 데이터 타입 강제 형변환(int/float)
- 뉴스 복구: 네이버 뉴스 최신순 수집 로직 유지
- 필드 매핑: KeyError('거래대금') 방지를 위한 다중 필드 매핑 적용
- 템플릿: 사용자 요청 일간 주가 리포트 양식 완벽 적용
"""
...
def fetch_data(ticker, dt):
    # 초기값 설정 (JSON 에러 방지를 위해 순수 파이썬 타입 사용)
    ...
    if not df_h.empty:
        ...
        # 이동평균 및 RSI
        res['ma'] = {
            "m5": int(c.rolling(5).mean().iloc[-1]), 
            "m10": int(c.rolling(10).mean().iloc[-1]), 
            "m20": int(c.rolling(20).mean().iloc[-1])
        }
